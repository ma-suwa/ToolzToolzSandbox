...
export const defaultParams = {
    NumbersOfShapes: { value: 18, type: "number", min: 1, max: 100, step: 1, label: "NumbersOfShapes/形の数" },
    Numbers: { value: 8, type: "number", min: 3, max: 100, step: 1, label: "Numbers/数" },
    ...
    Background: { value: "#9fbfad", type: "color", label: "Background/背景", category: "Color/色" },
    Random: { value: true, type: "boolean", label: "RandomLayout/ランダムな配置" },
    RandomMinScale: { value: 1, type: "number", min: 0, max: 1, step: 0.01, label: "MinScale/最小スケール" },
    RandomMaxScale: { value: 1, type: "number", min: 1, max: 3, step: 0.01, label: "MaxScale/最大スケール" },
    RandomOffset: { value: 100, type: "number", min: 0, max: 600, step: 1, label: "Offset/オフセット" },
    ...
    HueMax: { value: 30, type: "number", min: 0, max: 360, step: 1, label: "Max/最大値", category: "RandomColor/ランダムな色" },
    RandomSaturation: { value: true, type: "boolean", label: "Saturation/彩度", category: "RandomColor/ランダムな色" },
    SaturationMin: { value: 80, type: "number", min: 0, max: 100, step: 1, label: "Min/最小値", category: "RandomColor/ランダムな色" },
    ...
    LightnessMin: { value: 20, type: "number", min: 0, max: 100, step: 1, label: "Min/最小値", category: "RandomColor/ランダムな色" },
    LightnessMax: { value: 80, type: "number", min: 0, max: 100, step: 1, label: "Max/最大値", category: "RandomColor/ランダムな色" }
};
...
export const setupP5 = (p, params, container) => {
    ...
    const SHAPE_PARAMS = [
        'NumbersOfShapes', 'Width', 'Density', 'Random',
    ...
    const drawPolygon = (g, currentParams) => {
        ...
        for (let j = 0; j < sideNum; j++) {
            ...
            anchorPoints.push(g.createVector(
                centerX + g.cos(angleJ) * currentParams.Width,
                centerY + g.sin(angleJ) * currentParams.Width
            ));
            controlPoints.push(g.createVector(
                centerX + g.cos(angleJ) * controlDistance,
                centerY + g.sin(angleJ) * controlDistance
            ));
        }
